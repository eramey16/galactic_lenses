...
class Classifier:
        ...
        for _ in range(loop):
            try:
                with self.engine.connect() as conn:
                    ...
                    stmt = sa.select(bk_tbl).where(bk_tbl.c.ls_id==ls_id).where(bk_tbl.c.tag==tag)
    ...
    def ensure_db(self, ls_id=None, ra=None, dec=None, tag=None, train=False, 
        ...
        if not self.withmpi or self.rank==0:
            galaxy_in_db = False
            if ls_id is not None: # Search by ls_id
                result = self.get_galaxy(ls_id, tag=tag)
                if result is not None: galaxy_in_db = True
            if not galaxy_in_db: # Query galaxy from NOIRLab
                ...
                result = self.get_galaxy(ls_id, tag=tag)
                if result is not None: galaxy_in_db = True
            if not galaxy_in_db: # Insert galaxy into database
                if tbl_name is None: raise ValueError("Must supply table name for new galaxy")
